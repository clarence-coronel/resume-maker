...
type educationFields = {
  cancelHandler: () => any;
  isOpen: boolean;
  saveHandler: (toSave: education) => any;
  setIsAddActive: (state: boolean) => any;
};

type educationComponentProps = {
  educList: education[];
  add: (test: education) => any;
  ...
  return (
    <div className={`${isOpen ? "" : "hidden"} w-full gap-3 flex flex-col`}>
      <div className="flex flex-col items-start justify-center">
        ...
        <input
          id="school"
          className="focus:outline-blue-500 border-none bg-neutral-100 rounded-lg px-3 py-2 w-full h-fit"
          type="text"
      ...
      <div className="flex flex-col items-start justify-center">
        ...
        <input
          ...
          className="focus:outline-blue-500 border-none bg-neutral-100 rounded-lg px-3 py-2 w-full h-fit"
          type="text"
          placeholder="Bachelor in Example"
        />
      </div>
      ...
      <div className="flex flex-col-reverse flex-nowrap justify-end mt-8 gap-3 sm:flex-row">
        <button className="hidden hover:-translate-y-1 duration-200 flex-row flex-nowrap items-center justify-center gap-1 px-4 py-1 border-2 border-gray-200 text-gray-400 rounded-md font-bold">
          <svg
        ...
        <div className="flex flex-row flex-nowrap gap-3">
          <button
            ...
            className="hover:-translate-y-1 flex-1 duration-200 px-4 py-1 border-2 border-gray-200 text-gray-400 rounded-md font-bold"
          >
            Cancel
          </button>
          <button
            onClick={() => {
              saveHandler({
                id: uuid(),
                school:
                  document.querySelector<HTMLInputElement>("#school")?.value ||
                  "",
                degree:
                  document.querySelector<HTMLInputElement>("#degree")?.value ||
                  "",
                startDate:
            ...
            className="hover:-translate-y-1 flex-1 duration-200 px-4 py-1 text-white bg-blue-500 rounded-md font-bold"
          >
            Save
          </button>
        </div>
...
function clearEducFields(setIsAddActive: (active: boolean) => any): void {
  // Set the active state to false
  setIsAddActive(false);

  // Clear the values of the input fields safely
  const schoolInput = document.querySelector<HTMLInputElement>("#school");
  const degreeInput = document.querySelector<HTMLInputElement>("#degree");
